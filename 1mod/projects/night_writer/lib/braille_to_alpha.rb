...
class BrailleToAlpha < Translator
	...
	def translate_to_text(input)
      ...
      top = (braille.select.with_index{|_,index| index % 3 == 0}).join
      middle = (braille.select.with_index{|_,index| index % 3 == 1}).join
      bottom = (braille.select.with_index{|_,index| index % 3 == 2}).join
